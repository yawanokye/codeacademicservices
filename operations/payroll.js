...
function quantitySummary(row){return `<div class="quantity-stack"><span><b>${esc(row.claimedQuantity??row.claimedGroupsRaw)}</b> claimed</span><span><b>${esc(row.departmentApprovedQuantity)}</b> HoD payable</span><span><b>${esc(row.scoreSheetQuantity)}</b> approved scores</span>${row.paymentType?`<span>${esc(row.paymentType)}</span>`:''}${row.adjustmentReason?`<small>${esc(row.adjustmentReason)}</small>`:''}</div>`;}
function checkSummary(row){return `<div class="check-stack"><span class="${row.validation?.valid?'check-ok':'check-bad'}">${row.validation?.valid?'MATCH':'RECONCILED / PART PAYMENT'}</span>${row.claimFormPresent?`<button class="btn compact-btn" onclick="openClaim('${esc(row.id)}','${esc(row.reference)}')">Original claim${Number(row.claimFormCount)>1?` (${esc(row.claimFormCount)})`:''}</button>`:'<span class="check-bad">Claim form missing</span>'}${row.approvedClaimUrl?`<a class="btn compact-btn primary" href="${esc(row.approvedClaimUrl)}">Approved PDF</a>`:'<span class="check-bad">Approved PDF missing</span>'}<button class="btn compact-btn" onclick="openAudit('${esc(row.id)}','${esc(row.reference)}')">Audit history</button>${row.verificationCode?`<small>Code: ${esc(row.verificationCode)}</small>`:''}</div>`;}
function actionButtons(row){if((roleRank[identity.role]||0)<2)return '<span class="read-only">Read only</span>';return `<div class="payroll-row-actions"><button class="btn good" onclick="setStatus('${esc(row.id)}','verified')">Verify</button><button class="btn primary" onclick="setStatus('${esc(row.id)}','approved-for-payment')">Approve Payment</button><button class="btn good" onclick="setStatus('${esc(row.id)}','paid')">Mark Paid</button><button class="btn warn" onclick="setStatus('${esc(row.id)}','queried')">Query / Hold</button><button class="btn danger" onclick="setStatus('${esc(row.id)}','returned-to-department')">Return to Department</button><button class="btn reset-action" onclick="setStatus('${esc(row.id)}','pending')">Reset</button></div>`;}
function render(){
...
window.openAudit=async(id,reference)=>{try{const data=await get(`/api/admin/${encodeURIComponent(department)}/submissions/${encodeURIComponent(id)}/claim-audit`);document.getElementById('auditTitle').textContent=`Audit History · ${reference}`;document.getElementById('auditBody').innerHTML=(data.events||[]).slice().reverse().map(event=>`<article><strong>${esc(event.stage)} · ${esc(event.action)}</strong><small>${esc(fmt(event.at))} · ${esc(event.by||'System')}</small>${event.note?`<p>${esc(event.note)}</p>`:''}</article>`).join('')||'<p>No audit events recorded.</p>';document.getElementById('auditDialog').showModal()}catch(error){alert(error.message)}};
window.setStatus=async(id,status)=>{let note='';if(['queried','paid','approved-for-payment','returned-to-department'].includes(status)){const entered=prompt(status==='returned-to-department'?'Enter the reason for returning this claim to the department:':status==='queried'?'Enter query or hold reason:':status==='paid'?'Enter payment reference or note:':'Enter approval note:','');if(entered===null)return;note=entered.trim();if(['queried','returned-to-department'].includes(status)&&!note)return alert('A reason is required.')}try{await get(`/api/payroll/${encodeURIComponent(department)}/claims/${encodeURIComponent(id)}/status`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({status,note})});await load();}catch(error){alert(error.message);}};
...
document.getElementById('closeAudit').onclick=()=>document.getElementById('auditDialog').close();
