(function () {
  ...
  function feedbackPanel(ticket) {
    ...
    return `<section class="ticket-feedback"><h4>How was this service?</h4><p>Your feedback helps Student Support improve response quality. It does not change your right to raise a new matter.</p><form class="student-feedback-form"><div class="grid2"><div class="field"><label>Overall rating <span class="req">*</span></label><select name="rating" required><option value="">Choose 1 to 5</option><option value="5">5 — Excellent</option><option value="4">4 — Good</option><option value="3">3 — Fair</option><option value="2">2 — Poor</option><option value="1">1 — Very poor</option></select></div><div class="field"><label>Was the matter resolved? <span class="req">*</span></label><select name="resolved" required><option value="">Choose an answer</option><option value="yes">Yes</option><option value="partly">Partly</option><option value="no">No</option></select></div></div><div class="field"><label>What should we improve? <span class="optional-label">Optional unless rating 1–2</span></label><textarea name="comment" maxlength="2000"></textarea></div><button class="btn" type="submit">Submit feedback</button><div class="feedback-status status" role="status" aria-live="polite"></div></form></section>`;
  }
  ...
  async function submitFeedback(event) {
    ...
    const reference = ticketResult.querySelector('.ticket-ref-label')?.textContent || '';
    const body = { rating:Number(form.elements.rating.value), resolved:form.elements.resolved.value, comment:form.elements.comment.value, ...authFields() };
    button.disabled = true;
  ...
  function syncCategoryGuidance() {
    ...
    if (guidance) guidance.textContent = information?.evidenceGuidance || evidenceGuidance[category] || evidenceGuidance.general;
    const expectation = document.getElementById('serviceExpectation');
    if (expectation) expectation.innerHTML = information ? `<strong>Before submitting</strong><span>${esc(information.beforeSubmitting)}</span><small>Expected owner: ${esc(information.responsibleUnit)} · Standard target: ${esc(information.workingDays)} working day${Number(information.workingDays) === 1 ? '' : 's'}.</small>` : '';
  ...
  if (page.dataset.defaultType && document.getElementById('type')) document.getElementById('type').value = page.dataset.defaultType;
