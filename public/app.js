...
form.addEventListener('submit', async event => {
  ...
  try {
    ...
    const refText = result.reference ? ` Reference: ${result.reference}.` : '';
    const rowsText = Number.isFinite(Number(result.scoreRowsIncluded)) ? ` ${result.scoreRowsIncluded} score row(s) were added to the consolidated dataset.` : '';
    showFormMessage(`Submission completed successfully.${refText}${rowsText}`, 'success');
    form.reset();
    scoresAreValid = false;
    validatedRows = [];
    validationPanel.classList.add('hidden');
